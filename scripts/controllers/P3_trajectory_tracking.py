...
class TrajectoryTracker:
    ...
    def compute_control(self, x, y, th, t):
        ...
        if np.abs(self.V_prev)<V_PREV_THRES:
            self.reset() # Reset all values
            th_d = np.arctan(yd_d/xd_d)
            self.V_prev = xd_d/np.cos(th_d) if np.cos(th_d)>0.1 else yd_d/np.sin(th_d)

        xd = self.V_prev*np.cos(th)
        yd = self.V_prev*np.sin(th)

        u_1 = xdd_d+self.kpx*(x_d-x)+self.kdx*(xd_d-xd)
        u_2 = ydd_d+self.kpx*(y_d-y)+self.kdx*(yd_d-yd)

        u = np.array([u_1, u_2])

        top = np.array([np.cos(th), -self.V_prev*np.sin(th)])
        bottom = np.array([np.sin(th), self.V_prev*np.cos(th)])

        A = np.c_[top, bottom].T

        a, om = np.linalg.solve(A, u)

        # Update the velocity using rectangular integration
        V = self.V_prev + a*dt
        ########## Code ends here ##########
